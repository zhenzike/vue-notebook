...
即：在父组件使用插槽时，子组件可以事先准备好数据来供父组件使用【相当于子向父传值】，且这个值将以对象成员的形式传递，若不传值，则仅仅接收一个空对象，如果是以解构形式获取则直接获取到值，如：

```html
//父组件代码
<template #chacao="msg">
    <p>{{ msg }}</p>   //打印结果为：{'msg':'hello','user':"guo"} 【会以对象形式打印所有的属性值，若没有传值，打印结果为 {}】
</template>
...
<template #chacao="{ msg , user }">
    <p>{{ msg,user }}</p>   //打印结果为：hello 【若没有传值，打印结果为 {}】
</template>
```
